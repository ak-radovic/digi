...
TimeChangeRule tcr2 = {"tcr2", First, Sun, Apr, 10, 600};  // daylight/summer time, valid from last sunday of march at 2am, UTC + 2 hours (+120 minutes)
//-----------------------------------------------
...
uint8_t upperLimitLDR = 255;  // everything above this value will cause max brightness (according to current level) to be used (if it's higher than this)
...
float factorLDR = 1;                    // try 0.5 - 2.0, compensation value for avgLDR. Set dbgLDR true & define DEBUG and watch the serial monitor. Looking...
...
const uint8_t colorPreviewDuration = 0;                     // duration in seconds for previewing palettes/colorModes if colorPreview is enabled/true
const bool reverseColorCycling = false;                     // true = reverse color movements
const uint8_t brightnessLevels[11]{15, 75, 125, 175, 255};  // 0 - 255, brightness Levels (min, med, max) - index (0-2) will be saved to eeprom
uint8_t brightness = brightnessLevels[2];
// default brightness if none saved to eeprom yet / first run
#ifdef FADING
...
#endif
#define LED_PWR_LIMIT 900  // 500mA - Power limit in mA (voltage is set in setup() to 5v)
...
void readLDR() {  // read LDR value 5 times and write average to avgLDR
    ...
    if (runCounter == 5) {
        ...
        if (avgLDR >= upperLimitLDR && avgLDR < maxBrightness)
            avgLDR = maxBrightness;  // if avgLDR is above upperLimitLDR switch to max current brightness
